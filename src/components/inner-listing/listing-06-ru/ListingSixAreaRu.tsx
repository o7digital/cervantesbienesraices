...
const ListingSixAreaRu = () => {
  ...
  const typeOptionGroups = useMemo(() => {
    ...
    properties.forEach((prop) => {
      if (prop?.property_type) {
        ...
        if (normalized && !propertyTypes.has(normalized)) {
          const localized = localizePropertyTitle(prop.property_type.toString().trim(), "ru");
    ...
    if (operationTypes.size) {
      const operationOptions = Array.from(operationTypes.values()).map((operation) => ({
        ...
        label: operation === "venta" ? "Sale" : operation === "renta" ? "Rent" : operation,
      }));

      const hasSale = operationTypes.has("venta");
      if (hasSale) {
        operationOptions.push({ value: "operation:compra", label: "Purchase" });
      }
  ...
  const locationOptions = useMemo(() => {
    ...
    return Array.from(locations.values()).sort((a, b) =>
      normalizeText(a).localeCompare(normalizeText(b), "ru")
    );
  }, [properties]);
      ...
      <div className="container">
        ...
        <div className="row">
          ...
          {filteredProperties.map((prop) => {
            const localizedTitle = localizePropertyTitle(prop.title || "", "ru");
            return (
...
export default ListingSixAreaRu;
